import { Button } from "@/components/ui/button";
import { getPreview } from "@/lib/paths";
import { cn } from "@/lib/utils";
import {
  OneImageMetadata,
  OneSelectionResponse,
  QuestionProps,
} from "@/types/globals";
import Image from "next/image";
import { useState } from "react";

const OneImageBlog = ({ metadata, question }: QuestionProps) => {
  const [answer, setAnswer] = useState("");
  const correct = (metadata.correctAnswer as OneSelectionResponse)?.id;

  return (
    <div className="w-full h-full max-w-md mx-auto flex items-center justify-center flex-col gap-6">
      <h3 className="text-lg font-semibold first-letter:uppercase">
        {question}
      </h3>
      <div className="grid grid-cols-2 gap-2 w-full relative max-w-72">
        {(metadata as OneImageMetadata).data.map(({ id: itemId, imageUrl }) => {
          const isCorrect = correct === itemId;
          const image = getPreview(imageUrl, "quiz-images");
          return (
            <button
              className={cn(
                "rounded-xl border border-border/50 overflow-hidden ring-2 ring-transparent aspect-square transition-all cursor-pointer",
                answer &&
                  isCorrect &&
                  "border-green-600 ring-green-600 pointer-events-none",
                answer === itemId &&
                  !isCorrect &&
                  "border-red-600 ring-red-600 pointer-events-none"
              )}
              type="button"
              key={itemId}
              onClick={() => {
                setAnswer(itemId);
              }}
            >
              {image && (
                <Image
                  src={image}
                  alt={""}
                  width={400}
                  height={400}
                  className="w-full h-full object-cover"
                />
              )}
            </button>
          );
        })}
      </div>
      {!!answer && (
        <Button
          variant="link"
          type="button"
          className="mx-auto text-muted-foreground hover:no-underline"
          onClick={() => {
            setAnswer("");
          }}
        >
          Reset Answer
        </Button>
      )}
    </div>
  );
};

export default OneImageBlog;
