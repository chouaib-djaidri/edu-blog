...
import { TitleIdProps } from "@/types/globals";
import { CirclePlus, Trash2 } from "lucide-react";
import { useFieldArray, useFormContext } from "react-hook-form";

const OneOptionForm = ({ currentQuiz }: { currentQuiz: number }) => {
    ...
    <div className="w-full max-w-sm space-y-4">
      ...
      <div className="flex flex-col gap-2">
        {fields.map((field, index) => {
          const actualData = watch(
            `quizzes.${currentQuiz}.metadata.data.${index}`
          );
            ...
            <div key={field.id} className="flex items-center gap-2 relative">
              <div
                ...
                onClick={() => {
                  setValue(
                    `quizzes.${currentQuiz}.metadata.correctAnswer.id`,
                    watch(`quizzes.${currentQuiz}.metadata.data.${index}.id`)
                  );
                }}
              />
              <FormField
