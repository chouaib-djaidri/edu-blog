...
import { CircleArrowRight, CirclePlus, Trash2 } from "lucide-react";
import { useFieldArray, useFormContext } from "react-hook-form";

const MatchForm = ({ currentQuiz }: { currentQuiz: number }) => {
  const { control, watch, setValue } = useFormContext<TestFormValues>();
  const { fields, append, remove } = useFieldArray({
    control: control,
    name: `quizzes.${currentQuiz}.metadata.data.r`,
  }) as unknown as {
    fields: TitleIdProps[];
  ...
  const { append: lAppend, remove: lRemove } = useFieldArray({
    control: control,
    name: `quizzes.${currentQuiz}.metadata.data.l`,
  }) as unknown as {
    fields: TitleIdProps[];
    append: (value: TitleIdProps) => void;
    remove: (index: number) => void;
      ...
      <div className="flex flex-wrap gap-2 w-full max-w-lg">
        {fields.map((field, index) => (
          ...
          >
            <FormField
              control={control}
              name={`quizzes.${currentQuiz}.metadata.data.l.${index}.title`}
            ...
            <FormField
              control={control}
              name={`quizzes.${currentQuiz}.metadata.data.r.${index}.title`}
              render={({ field: titleField }) => (
                <FormItem className="flex-1">
