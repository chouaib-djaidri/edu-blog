...
import {
  reVerifyPasswordResetAction,
  verifyPasswordResetAction,
} from "@/actions/auth/reset-password-verification";
...
const ResetPasswordVerificationForm = ({
  ...
  const t = useTranslations("ResetPasswordVerification");
  ...
  const onSubmit = async (data: OTPVerificationFormValues) => {
    ...
    const state = await verifyPasswordResetAction(formData);
    if (state?.err) {
  ...
  const resendResetPasswordVerificationHandler = async () => {
    if (!!isPending) return;
    setIsPending("resend");
    const formData = new FormData();
    formData.set("email", email);
    const state = await reVerifyPasswordResetAction(formData);
    if (state?.err) {
      const isTimeError = state.err === "auth.tooManyRequests";
            ...
            <div className="flex flex-col gap-3 items-center">
              ...
              <div className="text-muted-foreground">
                ...
                <Button
                  ...
                  onClick={resendResetPasswordVerificationHandler}
                  type="button"
