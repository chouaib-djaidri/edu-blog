...
const FeedbackForm = () => {
  ...
  const form = useForm<FeedbackFormValues>({
    resolver: valibotResolver(FeedbackFormSchema(tv)),
      ...
      >
        <ScrollArea className="w-full h-full max-h-[calc(100svh-12rem)]">
          <div className="p-6 w-full flex flex-col gap-4">
            ...
            <div className="bg-background-alt border rounded-lg flex flex-col items-center p-8 w-full gap-4">
              ...
              render={({ field }) => (
                <FormItem className="flex-1">
                  ...
                  <FormControl>
                    <Textarea
                      {...field}
                      placeholder={tf("feedback.placeholder")}
...
function Emojies({ emoji }: { emoji: string }) {
  switch (emoji) {
    case "sobad":
      return (
        <svg
