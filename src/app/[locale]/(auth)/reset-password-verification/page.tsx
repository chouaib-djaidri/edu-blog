import ResetPasswordVerificationForm from "@/components/forms/auth/reset-password-verification-form";
...
const ResetPasswordVerificationPage = async ({
  ...
  const { data: userData } = await supabase.rpc("check_reset_password_token", {
    user_email: email,
  });
  ...
  return (
    <main>
      <div className="min-h-[100dvh] flex flex-col items-center justify-center gap-0 py-6">
        <ResetPasswordVerificationForm email={email} restTime={restTime || 0} />
      </div>
    </main>
  );
};
